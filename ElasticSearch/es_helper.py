...
# Default Schema Mapping configuration
INDEX_MAPPING = {
    "mappings": {
        "properties": {
            "document_id": {
...
def get_elasticsearch_client(
    ...
    """
    Initializes and returns an official Elasticsearch client.
    Supports Basic Auth, API Key authentication, SSL/TLS validation, and retry parameters.
    """
    if not hosts:
        # Fallback to env or default local address
...
def initialize_index(es_client: Elasticsearch, index_name: str = "svo_chunks") -> None:
    """
    Checks if index exists, and creates it with settings & mappings if missing.
    Allows configuring shards and replicas via environment variables.
    """
    try:
        ...
        logger.error(f"Failed to create index due to request/schema error: {e}")
        raise
...
def generate_bulk_actions(chunks: List[Any], index_name: str) -> Any:
    """
    Generator yielding bulk index operations from list of Chunk objects.
    """
    for chunk in chunks:
        yield {
            "_index": index_name,
...
def bulk_ingest_chunks(es_client: Elasticsearch, chunks: List[Any], index_name: str = "svo_chunks") -> Dict[str, Any]:
    """
    Ingests chunk objects into Elasticsearch using the resilient helpers.bulk API.
